...
class ChatActivity : BaseActivity() {
    ...
    private fun updateHeaderUI() {
        ...
        if (isGroup) {
            ...
            ivChatAvatar.setOnClickListener(null)
    ...
    private fun showSwitchSenderDialog(msg: DirectMessage, position: Int) {
        val participants = if (isGroup) {
            val group = chatGroups.find { it.id == chatId } ?: return
            people.filter { group.participantIds.contains(it.id) && !it.isArchived }
        } else {
            val ids = msg.chatId.split("_")
            people.filter { ids.contains(it.id) }
        }
    ...
    private fun showSwitchActiveUserDialog() {
        val participants = if (isGroup) {
            ...
            val ids = listOf(currentUser.id, chatId!!).sorted()
            people.filter { ids.contains(it.id) && !it.isArchived }
        }
        val names = participants.map { it.sysmediaProfile?.displayName ?: it.name }.toTypedArray()
        ...
        AlertDialog.Builder(this)
            .setTitle(R.string.action_switch_account)
            .setItems(names) { _, which ->
                val newActiveUser = participants[which]
                if (newActiveUser.id != currentUser.id) {
                    if (!isGroup) {
                        val oldActiveId = currentUser.id
                        currentUser = newActiveUser
                        chatId = oldActiveId
    ...
    private inner class ChatAdapter(var items: List<DirectMessage>) :
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            ...
            if (msg.imageUri != null) {
                ...
                holder.ivImage.layoutParams.width = ViewGroup.LayoutParams.WRAP_CONTENT
                holder.cardBubble.layoutParams.width = ViewGroup.LayoutParams.WRAP_CONTENT
            }
            ...
            val lpRow = holder.layoutBubbleRow.layoutParams as LinearLayout.LayoutParams
            val lpSender = holder.tvSender.layoutParams as LinearLayout.LayoutParams
                            ...
                            when (selectedOption) {
                                ...
                                getString(R.string.delete) -> {
                                    AlertDialog.Builder(this@ChatActivity)
                                        ...
                                        .show()
                                        .let { dialog: AlertDialog -> ColorHelper.styleAlertDialog(dialog, this@ChatActivity) }
                                }
                            }
                        }.show()
                        .let { ColorHelper.styleAlertDialog(it, this@ChatActivity) }
                }
                true
            }
        }
