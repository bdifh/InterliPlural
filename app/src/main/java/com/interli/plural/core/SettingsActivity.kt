...
import com.interli.plural.core.CustomFieldsActivity
...
import com.interli.plural.core.ThemesActivity
...
import com.interli.plural.features.mood.MoodThemesActivity
...
private var pendingPdfSelections: BooleanArray? = null
    ...
    private fun findMemberMatching(people: List<Person>, id: String?, name: String?): Person? {
        if (!id.isNullOrEmpty()) {
            val byManualId = people.find { it.manualId == id }
            if (byManualId != null) return byManualId
            val byId = people.find { it.id == id }
            if (byId != null) return byId
        }
        if (!name.isNullOrEmpty()) {
            val byName = people.find { it.name.equals(name, ignoreCase = true) }
            if (byName != null) return byName
        }
        return null
    }

    private fun processFrontHistory(historyData: List<Map<String, Any>>, people: List<Person>) {
                ...
                if (meConn.responseCode == 200) {
                    ...
                    if (userId != null) {
                        ...
                        if (conn.responseCode == 200) {
                            ...
                            val hUrl = URL("https://api.apparyllis.com/v1/fronthistory/$userId")
                            val hConn = hUrl.openConnection() as HttpURLConnection
                            hConn.setRequestProperty("Authorization", token)
                            if (hConn.responseCode == 200) {
                                val hJson = hConn.inputStream.bufferedReader().use { it.readText() }
        ...
        Thread {
            try {
                ...
                if (membersList != null) {
                    ...
                    val historyList = root["frontHistory"] as? List<Map<String, Any>>
                    if (historyList != null) {
                        processFrontHistory(historyList, people)
                    }

                    runOnUiThread {
                        Toast.makeText(this, "Import voltooid: $mCount leden en geschiedenis verwerkt!", Toast.LENGTH_LONG).show()
                        recreate()
                    }
                }
            } catch (e: Exception) {
                runOnUiThread { Toast.makeText(this, "Fout: ${e.message}", Toast.LENGTH_SHORT).show() }
            }
        }.start()
    }
    ...
    private fun processHivemindImport(root: Map<String, Any>) {
        ...
        hmAlters.forEach { alter ->
            ...
            if (hmId.isNotEmpty() && people.none { it.manualId == hmId }) {
                ...
                val colorHex = alter["color"] as? String
                val avatarData = alter["avatar_data"] as? String
                var profileColor = -6934396
                if (!colorHex.isNullOrBlank()) {
                    try {
                        profileColor = android.graphics.Color.parseColor(colorHex)
